...
# db = client["digital_signature_db"]  # Use your database name
...
def generate_and_store_keys(student_id):
    """
    Generate RSA private and public keys, save them to the 'keys' folder,
    and store them in MongoDB for a specific student.
    ...
    # Check if the student's keys already exist in MongoDB
    if students_collection.find_one({"student_id": student_id}):
        print(f"⚠️ Keys for student '{student_id}' already exist in MongoDB.")
        return
    ...
    try:
        ...
        # Save Private Key to the 'keys' folder
        private_key_path = f"keys/{student_id}_private_key.pem"
        with open(private_key_path, "wb") as private_file:
            private_file.write(private_key_pem)

        # Save Public Key to the 'keys' folder
        public_key_path = f"keys/{student_id}_public_key.pem"
        with open(public_key_path, "wb") as public_file:
            public_file.write(public_key_pem)

        print(f"✅ Keys have been successfully saved to the 'keys' folder for {student_id}.")
...
def check_student_in_mongo(student_id):
    ...
    if count == 1:
        ...
        print(f"⚠️ Multiple entries found for student '{student_id}' in MongoDB!")
        return False
    else:
        print(f"❌ Student '{student_id}' does not exist in MongoDB.")
        return False
...
# Example: Add a new student and check if it's unique
if __name__ == "__main__":
    new_student_id = "Samarth"

    # Generate keys for Samarth and store them in MongoDB
    generate_and_store_keys(new_student_id)

    # Verify if Samarth is unique in MongoDB
    is_unique = check_student_in_mongo(new_student_id)
